...
> **Engine-specialist agents removed.** This project is TypeScript + Phaser (web), so the
> Godot / Unity / Unreal lead and sub-specialist agents were deleted from `.claude/agents/`.
> Engine-level work is handled by the engine-agnostic `engine-programmer` and `gameplay-programmer`.
